    ...
    def create(self, name, version, is_preview_only, configuration, update_if_exists=False):
        if update_if_exists:
            ...
            if len(existing_report) > 0:
                ...
                return self.update(existing_report["id"], {"version": version, "configuration": json.dumps(configuration), "isPreview": is_preview_only})

        # create a new one
        url = self.create_url("/report")
        fields = {"version": version, "configuration": json.dumps(configuration), "name": name, "isPreview": is_preview_only}
        response = requests.post(url, headers={"Authorization": "bearer " + self.access_token}, json=fields)
        if response.status_code != 200:
            raise Exception("Could not create report: " + str(response.text.replace("\\n", "\n")))
