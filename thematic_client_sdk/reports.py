...
class Reports(Requestor):
    def get(self, report_id=None):
        ...
        if response.status_code != 200:
            raise ThematicAPIError(
                "Could not retrieve reports: " + str(response.text),
    ...
    def create(
        self, name, version, is_preview_only, configuration, update_if_exists=False
    ):
        if update_if_exists:
            reports = self.get()
        ...
        if response.status_code != 200:
            raise ThematicAPIError(
                "Could not create report: " + str(response.text.replace("\\n", "\n")),
                status_code=response.status_code,
                response_text=response.text,
            )
    ...
    def update(self, report_id, fields):
        ...
        if response.status_code != 200:
            raise ThematicAPIError(
                "Could not update report: " + str(response.text),
                status_code=response.status_code,
                response_text=response.text,
            )
